...
class TaskRunner:
    ...
    def run(self):
        ...
        try:
            ...
            if self.should_stop:
                ...
                db.update_task_status(
                    self.task_id, status, exit_code=exit_code, stdout=stdout, stderr=stderr, finished_at=finished_at
                )
            else:
    ...
    def _execute_ssh_command(self, server):
        ...
        try:
            ...
            stdin, stdout, stderr = self.ssh_client.exec_command(self.task["command"], timeout=timeout)  # nosec B601
...
def enqueue_task(task_id):
    ...
    try:
        ...
        db.update_task_status(task_id, "failed", exit_code=-1, stderr="Task queue is full. Please try again later.")
        return False
    except Exception as e:
        # Unexpected error
        print(f"Error enqueuing task {task_id}: {e}")
        db.update_task_status(task_id, "failed", exit_code=-1, stderr=f"Failed to enqueue task: {str(e)}")
        return False
