...
class SSHKeyManager:
    ...
    def create_key(self, name: str, private_key: str, description: str = None, user_id: int = None) -> Dict:
        """
        ...
        try:
            ...
            for row in rows:
                ...
                if row["created_by_user_id"]:
                    cursor.execute("SELECT username FROM users WHERE id = ?", (row["created_by_user_id"],))
    ...
    def get_decrypted_key(self, key_id: str) -> Optional[str]:
        ...
        try:
            ...
            # Decrypt private key
            vault = get_vault()
            private_key = vault.decrypt(row["private_key_enc"], row["iv"], row["auth_tag"])
