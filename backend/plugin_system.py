    ...
    def _load_plugin(self, plugin_name: str) -> None:
        ...
        if config_str:
            try:
                ...
                logger.warning(f'Failed to parse plugin config: {plugin_name}',
    ...
    def startup(self, ctx: Optional[Dict[str, Any]] = None) -> None:
        ...
        for plugin_name, plugin in self.plugins.items():
            ...
            try:
                ...
                logger.info(f'Plugin startup: {plugin_name}',
                           service=f'plugin:{plugin_name}')
            except Exception as e:
                logger.error(f'Plugin startup error: {plugin_name}',
                           error=str(e),
                           service=f'plugin:{plugin_name}')
    ...
    def shutdown(self) -> None:
        ...
        for plugin_name, plugin in self.plugins.items():
            try:
                ...
                logger.error(f'Plugin shutdown error: {plugin_name}',
                           error=str(e),
                           service=f'plugin:{plugin_name}')
