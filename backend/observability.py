...
class StructuredLogger:
    ...
    def request(self, method: str, path: str, status_code: int, latency_ms: float, 
    ...
    def to_prometheus(self) -> str:
        ...
        for endpoint, latencies in self.request_latency.items():
            if latencies:
                ...
                lines.append(f'server_monitor_request_latency_ms{{endpoint="{sanitized}",quantile="0.5"}} {round(avg, 2)}')
                if len(latencies) > 1:
                    p95 = sorted(latencies)[int(len(latencies) * 0.95)]
                    lines.append(f'server_monitor_request_latency_ms{{endpoint="{sanitized}",quantile="0.95"}} {round(p95, 2)}')
    ...
    def readiness() -> Dict[str, Any]:
        ...
        try:
            if db is None:
                ...
                if cursor.fetchone():
                    checks['database'] = {
                        'status': 'ok',
                        'message': 'Database readable and tables exist'
                    }
                else:
                    checks['database'] = {
                        'status': 'error',
                ...
                try:
                    ...
                    checks['database_write'] = {
                        'status': 'error',
                        'message': f'Database not writable: {str(e)}'
                    }
                    overall_status = 'not_ready'
        ...
        if vault_key and len(vault_key) >= 32:
            checks['vault_master_key'] = {
                'status': 'ok',
                'message': 'Vault master key configured'
            }
        else:
            checks['vault_master_key'] = {
                'status': 'warning',
        ...
        if jwt_secret and len(jwt_secret) >= 32:
            ...
            checks['jwt_secret'] = {
                'status': 'warning',
                'message': 'JWT secret not configured or too short'
            }
