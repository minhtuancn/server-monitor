...
def get_remote_agent_data_with_timeout(
    host, port, username, agent_port=8083, endpoint="/api/all", ssh_key_path=None, password=None, timeout=10
):
    """
    ...
    try:
        ...
        future = ssh_pool.executor.submit(
            get_remote_agent_data, host, port, username, agent_port, endpoint, ssh_key_path, password
        )
...
def check_port_available(host, port, username, check_port, ssh_key_path=None, password=None):
    ...
    try:
        ...
        if "PORT_AVAILABLE" in output or not output:
            return {"success": True, "available": True, "port": check_port, "process": None, "pid": None}
        else:
            # Parse process info
            import re
    ...
    try:
        for check_port in range(start_port, start_port + 100):
            ...
            if result.get("success") and result.get("available"):
                return {"success": True, "suggested_port": check_port, "message": f"Port {check_port} is available"}
