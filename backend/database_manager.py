...
class DatabaseManager:
    ...
    def create_backup(self) -> Dict:
        ...
        """
        try:
            result = subprocess.run([str(BACKUP_SCRIPT), "backup"], capture_output=True, text=True, timeout=60)
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "message": "Backup created successfully", "backup": latest}
            else:
                return {"success": False, "message": f"Backup failed: {result.stderr}", "error": result.stderr}
        except subprocess.TimeoutExpired:
            return {"success": False, "message": "Backup timed out", "error": "Operation took longer than 60 seconds"}
        except Exception as e:
            return {"success": False, "message": f"Backup error: {str(e)}", "error": str(e)}
            ...
            if metadata_file.exists():
                try:
                    with open(metadata_file, "r") as f:
                        ...
                        backup_info.update(
                            {"checksum": metadata.get("checksum_sha256"), "timestamp": metadata.get("timestamp")}
                        )
                except Exception:
                    pass
    ...
    def delete_backup(self, filename: str) -> Dict:
        ...
        try:
            ...
            return {"success": False, "message": f"Failed to delete backup: {str(e)}", "error": str(e)}
    ...
    def restore_backup(self, filename: str) -> Dict:
        ...
        try:
            result = subprocess.run(
                [str(RESTORE_SCRIPT), "restore", filename], capture_output=True, text=True, timeout=120
            )
            ...
            if result.returncode == 0:
                ...
                return {"success": False, "message": f"Restore failed: {result.stderr}", "error": result.stderr}
        except subprocess.TimeoutExpired:
            return {"success": False, "message": "Restore timed out", "error": "Operation took longer than 120 seconds"}
        except Exception as e:
            return {"success": False, "message": f"Restore error: {str(e)}", "error": str(e)}
    ...
    def check_health(self) -> Dict:
        ...
        try:
            ...
            return {"healthy": False, "error": str(e), "last_checked": datetime.now().isoformat()}
    ...
    def get_storage_stats(self) -> Dict:
        ...
        try:
            ...
            return {
                "database": {"size": db_size, "size_human": self._format_size(db_size)},
                "backups": {
                    "count": backup_count,
                ...
                "data_directory": {"total_size": data_total_size, "size_human": self._format_size(data_total_size)},
            }
        except Exception as e:
            return {"error": str(e)}
