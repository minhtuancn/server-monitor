...
DEFAULT_THRESHOLDS = {"cpu": 90, "memory": 85, "disk": 90}  # CPU usage > 90%  # Memory usage > 85%  # Disk usage > 90%
...
def send_alert(server_name, alert_type, message, severity="warning", server_id=None):
    ...
    if enabled_channels.get("email"):
        ...
        try:
            email_result = email.send_alert_email(
                server_name=server_name, alert_type=alert_type, message=message, severity=severity, server_id=server_id
            )
    ...
    if server_id:
        try:
            alert_id = db.create_alert(server_id=server_id, alert_type=alert_type, message=message, severity=severity)
    ...
    if channel in ["email", "all"]:
        try:
            results["email"] = email.send_alert_email(
                server_name=server_name, alert_type=alert_type, message=message, severity=severity
            )
        except Exception as e:
