...
def recover_stale_tasks():
    ...
    try:
        ...
        if not running_tasks:
            ...
            return {
                'recovered': 0,
                'message': 'No running tasks to recover'
            }
        ...
        if recovered_count > 0:
            try:
                ...
                logger.warning(
                    'Failed to create audit log for task recovery',
                    error=str(e)
                )
        ...
        return {
            'recovered': 0,
            'error': str(e),
            'message': f'Task recovery failed: {str(e)}'
        }
...
def recover_terminal_sessions():
    ...
    try:
        ...
        if not active_sessions:
            ...
            return {
                'recovered': 0,
                'message': 'No active terminal sessions to recover'
            }
        ...
        logger.info(
            'Terminal session recovery complete',
            recovered=recovered_count,
        ...
        return {
            'recovered': 0,
            'error': str(e),
            'message': f'Terminal session recovery failed: {str(e)}'
        }
...
def run_startup_recovery():
    ...
    return {
        'tasks': task_result,
        'terminal_sessions': session_result,
        'total_recovered': total_recovered
    }
