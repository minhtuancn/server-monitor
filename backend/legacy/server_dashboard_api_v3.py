...
def get_system_info():
    ...
    return {
        "memory": {"total": total_mem, "used": used_mem, "free": free_mem, "percent": mem_percent},
        "disk": {"total": disk_total, "used": disk_used, "free": disk_free, "percent": disk_percent},
        "cpu": {"count": int(cpu_count) if cpu_count else 1, "load_avg": load_avg, "usage": cpu_val},
        "uptime": uptime_str,
        "hostname": hostname,
    }
...
def get_network_stats():
    ...
    # Store in history
    history_data["network"].append({"time": current_time, "rx": rx_speed, "tx": tx_speed})

    last_network_stats = {"rx_bytes": total_rx, "tx_bytes": total_tx, "timestamp": current_time}
...
def get_disk_io():
    ...
    last_disk_io = {"read_bytes": read_bytes, "write_bytes": write_bytes, "timestamp": current_time}

    history_data["disk_io"].append({"time": current_time, "read": read_speed, "write": write_speed})
...
def get_ports():
    ...
    for line in output.split("\n"):
        ...
        if match:
            ...
            port_info = PORT_DESCRIPTIONS.get(port, {"name": f"Port {port}", "desc": "Dịch vụ không xác định"})
...
def get_security_info():
    ...
    for line in ssh_sessions.split("\n"):
        if line:
            ...
            if len(parts) >= 5:
                sessions.append(
                    {"user": parts[0], "tty": parts[1], "ip": parts[4].strip("()") if len(parts) > 4 else "local"}
                )
...
def get_quick_stats():
    ...
    return {
        "top_memory_process": (
            {"pid": top_mem["pid"], "command": top_mem["command"][:50], "mem": top_mem["mem"]} if top_mem else None
        ),
        "top_cpu_process": (
            {"pid": top_cpu["pid"], "command": top_cpu["command"][:50], "cpu": top_cpu["cpu"]} if top_cpu else None
        ),
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path == "/api/process/kill":
                ...
                self._set_headers()
                self.wfile.write(
                    json.dumps({"success": True, "message": f"Sent SIGTERM to process {pid} ({check})"}).encode()
                )
                ...
                self._set_headers()
                self.wfile.write(
                    json.dumps({"success": True, "message": f"{action.title()}ed service {service}"}).encode()
                )
                ...
                self._set_headers()
                self.wfile.write(
                    json.dumps({"success": True, "message": f"{action.title()}ed container {container_id}"}).encode()
                )
                ...
                self._set_headers()
                self.wfile.write(json.dumps({"success": True, "message": "Cleared system cache"}).encode())
