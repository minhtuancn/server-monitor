...
def send_slack_message(webhook_url, payload):
    ...
    if not webhook_url or not webhook_url.startswith('https://hooks.slack.com/'):
        return {'success': False, 'error': 'Invalid Slack webhook URL. Must be HTTPS and start with https://hooks.slack.com/'}
...
def send_alert_slack(server_name, alert_type, message, severity='warning', server_id=None):
    ...
    if server_id:
        slack_message['attachments'][0]['fields'].insert(1, {'title': 'Server ID', 'value': str(server_id), 'short': True})
